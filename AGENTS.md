...
├── .env / .env.example
├── app/
│   ├── main.py                # FastAPI + lifespan（DB 初始化、调度器启停）+ 页面路由
...
│   │   ├── servers.py         # 服务器 CRUD + 健康/趋势/日志/执行/诊断/告警
│   │   ├── dashboard.py       # 仪表盘聚合
│   │   ├── timeline.py        # 事件时间线
│   │   └── knowledge.py       # 知识库管理
│   ├── services/              # 7 个服务模块
...
│   │   ├── log_filter.py      # 智能日志过滤（14 条正则 + 上下文 + 去重）
│   │   ├── ssh.py             # SSH 连接 + 命令执行 + 凭证加解密
│   │   ├── monitor.py         # 健康指标采集 + 解析 + AI 分析
...
8. **Webhook 双格式** — URL 含 `feishu` 发飞书交互式卡片，否则钉钉 Markdown。
9. **定时调度** — APScheduler 每分钟扫描 `schedule_interval > 0` 的在线服务器。
10. **前端零构建** — vanilla JS + CSS，Chart.js 和 GSAP 通过 CDN 引入。
11. **GSAP 动画** — 卡片 stagger 入场、弹窗缩放、Toast 滑入，尊重 `prefers-reduced-motion`。
12. **操作审计** — 所有远程命令存入 execution_logs，可追溯。
13. **ponytail: 服务器分组暂未开放**，后端代码已移除。
