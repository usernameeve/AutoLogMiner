...
├── .env / .env.example
├── app/
│   ├── main.py                # FastAPI + lifespan (DB/调度器启停) + 页面路由
│   ├── config.py              # 配置 + Fernet 密钥自动生成
│   ├── db.py                  # 6 张表 CRUD + 数据清理
│   ├── routes/                # 7 个路由模块 (46 个端点)
...
│   │   ├── prompt.py          # 提示词 + 知识库注入
│   │   ├── log_filter.py      # 智能日志过滤 (14 条正则 + 上下文 + 去重 + Token 兜底)
│   │   ├── ssh.py             # SSH 连接 + 3 次指数退避重试 + Fernet 加解密
│   │   ├── monitor.py         # 健康指标采集脚本 + 输出解析 + AI 分析 (异常降级)
│   │   ├── scheduler.py       # APScheduler 定时并发检查 + 每日数据清理
│   │   └── alerting.py        # 阈值比对 + 冷却期 + 双格式 Webhook + 失败日志
│   ├── models/schemas.py      # Pydantic 模型 (含 api_key 脱敏)
│   ├── static/
│   │   ├── app.js             # 全量前端 (仪表盘/图表/服务器/告警/知识库/时间线/自动刷新)
│   │   └── style.css          # 全量样式
...
1. **SSH 即用即关 + 重试** — 每次命令新建连接，用完关闭。连接失败自动重试 3 次，指数退避 1s/2s/4s，连续失败才标记 offline。
2. **凭证加密** — SSH 密码 Fernet 加密存储，密钥首次启动自动写入 .env。
3. **健康检查一体脚本** — 一个 Shell 脚本采集 6 类指标，一次 SSH 调用完成。LLM 不可用时自动降级，指标正常入库。
4. **智能日志过滤** — 14 条正则匹配信号行 -> 上下文保留 -> 去重 -> Token 截断兜底。
5. **多文件 Glob 支持** — `tail -q` 合并 logrotate 轮转文件，一次分析完整故障链。
6. **LLM 供应商路由** — `get_client(provider_id)` 按需解析凭证。
7. **告警冷却期** — 同一指标 30 分钟内不重复推送 (ALERT_COOLDOWN_MINUTES 可配)。
8. **Webhook 双格式** — URL 含 feishu 发飞书交互式卡片 (红色/黄色标题栏)，否则钉钉 Markdown。
9. **定时调度并发** — APScheduler 每分钟用 asyncio.gather 并发扫描所有在线服务器。
10. **数据自动清理** — 每日清理 >30 天的 health_checks 和 execution_logs。
11. **危险命令拦截** — execute 端点拦截 rm -rf /、mkfs.、dd if=、shutdown、reboot、Fork 炸弹等 9 条。
12. **前端零构建** — vanilla JS + CSS，Chart.js 和 GSAP 通过 CDN 引入。
13. **GSAP 动画** — 卡片 stagger 入场、弹窗缩放、Toast 滑入，尊重 prefers-reduced-motion。
14. **操作审计** — 所有远程命令存入 execution_logs，可追溯。
